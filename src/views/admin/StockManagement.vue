      ...
      <div class="requests-grid">
        ...
        >
          ...
          <div v-if="request.status !== 'pending'" class="admin-response">
            ...
            <small v-if="request.response_date">{{
        ...
        <table class="inventory-table">
          ...
          <tbody>
            <tr v-for="item in filteredInventory" :key="item.id">
              <td>
                <div class="item-info">
                  <div
                    ...
                    :style="{ background: item.item_color }"
                  >
              ...
              <td
                :class="{
                  critical: item.quantity <= item.low_stock_threshold * 0.3,
                  low: item.quantity <= item.low_stock_threshold,
                }"
              >
                {{ item.quantity }}
              </td>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            />
            <small
              >Available in admin inventory:
              {{ getAvailableStock(selectedRequest?.item_id) }}</small
            >
          </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="selected-item" v-if="selectedInventoryItem">
            <div
              class="item-icon"
              :style="{ background: selectedInventoryItem.item_color }"
            >
            ...
            <div>
              ...
              <div class="item-detail">
                Current Stock: {{ selectedInventoryItem.quantity }} units
              </div>
              <div class="item-detail">
                Threshold: {{ selectedInventoryItem.low_stock_threshold }}
              </div>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="adjustment-types">
              <label class="radio-label">
                <input type="radio" v-model="adjustmentType" value="add" /> Add
                Stock
              </label>
              <label class="radio-label">
              ...
              <label class="radio-label">
                <input type="radio" v-model="adjustmentType" value="set" /> Set
                New Value
              </label>
            </div>
          </div>
      ...
      if (requestSearch.value) {
        filtered = filtered.filter(
          (r) =>
            r.item_name
              ?.toLowerCase()
              .includes(requestSearch.value.toLowerCase()) ||
            r.requested_by
              ?.toLowerCase()
              .includes(requestSearch.value.toLowerCase()),
        );
    ...
    const filteredMovements = computed(() => {
      ...
      if (movementSearch.value) {
        filtered = filtered.filter((m) =>
          m.item_name
            ?.toLowerCase()
            .includes(movementSearch.value.toLowerCase()),
        );
      }
      if (movementTypeFilter.value) {
        filtered = filtered.filter(
          (m) => m.movement_type === movementTypeFilter.value,
        );
      }
      if (fromDate.value) {
